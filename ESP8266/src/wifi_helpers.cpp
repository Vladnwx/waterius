        ...
        if (WiFi.isConnected())
        {
            ...
            if (sett.mdns_on)
            {
                ...
                if (MDNS.begin(get_device_name()))
                {
                    ...
                    // Поиск mDNS сервиса и автонастройка HTTP выполняется только:
                    // - в режиме настройки (SETUP_MODE)
                    // - при первом запуске после настройки (setup_finished_counter == 0)
                    // - при ручной отправке данных (MANUAL_TRANSMIT_MODE) и если http_url не настроен
                    bool should_discover = (sett.mode == SETUP_MODE) 
                                           //|| (sett.setup_finished_counter == 0)
                                           //|| (sett.mode == MANUAL_TRANSMIT_MODE && sett.http_url[0] == '\0')
                                           ;
                    ...
                    if (should_discover)
                    {
                        ...
                        if (n > 0)
                        {
                            ...
                            LOG_INFO(F("WIFI: MDNS: Auto-configured http_url: ") << sett.http_url);
                        }
                        else
                        {
                    ...
                    {
                        LOG_INFO(F("WIFI: MDNS: Skipping service discovery (not in setup mode)"));
                    }
                }
                else
                {
